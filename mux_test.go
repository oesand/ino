...
func TestNew(t *testing.T) {
	m := mo.New(
		mo.Get("/hello", simpleHandler("hello")),
		mo.Get("/world", simpleHandler("world")),
	)
	if m == nil {
...
func TestInclude(t *testing.T) {
	m := mo.New()
	m.Include(
		mo.Get("/test", simpleHandler("test")),
		mo.Post("/submit", simpleHandler("submit")),
	)
...
func TestRouteCreation(t *testing.T) {
	tests := []struct {
		...
		{"Get", mo.Get("/test", simpleHandler("test")), http.MethodGet},
		{"Post", mo.Post("/test", simpleHandler("test")), http.MethodPost},
		{"Put", mo.Put("/test", simpleHandler("test")), http.MethodPut},
		{"Delete", mo.Delete("/test", simpleHandler("test")), http.MethodDelete},
		{"Options", mo.Options("/test", simpleHandler("test")), http.MethodOptions},
		{"Head", mo.Head("/test", simpleHandler("test")), http.MethodHead},
		{"Connect", mo.Connect("/test", simpleHandler("test")), http.MethodConnect},
		{"Patch", mo.Patch("/test", simpleHandler("test")), http.MethodPatch},
		{"Trace", mo.Trace("/test", simpleHandler("test")), http.MethodTrace},
	}
...
func TestRouteAttributes(t *testing.T) {
	attr1 := "auth_required"
	attr2 := 42
	route := mo.Get("/protected", simpleHandler("test"), attr1, attr2)
	attrs := route.Attrs()
	if len(attrs) != 2 {
...
func TestHandleInvalidPattern(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mo.Get(tt.pattern, simpleHandler("test"))
		})
	}
}
...
func TestHandleNilHandler(t *testing.T) {
	...
	mo.Handle(http.MethodGet, "/test", nil)
}

func TestServeHTTP(t *testing.T) {
	m := mo.New(
		mo.Get("/", simpleHandler("root")),
		mo.Get("/hello", simpleHandler("hello")),
		mo.Post("/submit", simpleHandler("submit")),
	)
...
func TestPathParameters(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := mo.New(
				mo.Get(tt.pattern, mo.F(func(w http.ResponseWriter, r *http.Request) {
					params := mo.PathParams(r.Context())
...
func TestMatchedRoute(t *testing.T) {
	m := mo.New(
		mo.Get("/test", mo.F(func(w http.ResponseWriter, r *http.Request) {
			route := mo.MatchedRoute(r.Context())
			if route == nil {
...
func TestPrefixRoutes(t *testing.T) {
	routes := mo.PrefixRoutes("/api",
		mo.Get("/users", simpleHandler("users")),
		mo.Get("/posts", simpleHandler("posts")),
	)
	m := mo.New(routes...)
...
func TestNestedPrefixRoutes(t *testing.T) {
	routes := mo.PrefixRoutes("/api",
		mo.PrefixRoutes("/v1",
			mo.Get("/users", simpleHandler("v1_users")),
		),
		mo.PrefixRoutes("/v2",
			mo.Get("/users", simpleHandler("v2_users")),
		),
	)
	m := mo.New(routes...)
...
func TestMiddlewareOrder(t *testing.T) {
	...
	mw1 := mo.Middleware(func(w http.ResponseWriter, r *http.Request, next http.Handler) {
		callOrder = append(callOrder, "middleware1")
		next.ServeHTTP(w, r)
	})
	m.Middleware(mw1)
	mw2 := mo.Middleware(func(w http.ResponseWriter, r *http.Request, next http.Handler) {
...
func TestMiddlewareShortCircuit(t *testing.T) {
	...
	blocker := mo.Middleware(func(w http.ResponseWriter, r *http.Request, next http.Handler) {
...
func TestRoutesIterator(t *testing.T) {
	m := mo.New(
		mo.Get("/a", simpleHandler("a")),
		mo.Post("/b", simpleHandler("b")),
		mo.Put("/c", simpleHandler("c")),
	)
...
func TestRouteSorting(t *testing.T) {
	...
	m := mo.New(
		mo.Get("/posts", simpleHandler("list")),
		mo.Get("/posts/{id}", simpleHandler("detail")),
		mo.Get("/posts/{id}/comments", simpleHandler("comments")),
	)
...
func TestMultipleRouteRegistration(t *testing.T) {
	...
	m.Include(
		mo.Post("/hello", simpleHandler("posted")),
		mo.Put("/hello", simpleHandler("updated")),
	)
